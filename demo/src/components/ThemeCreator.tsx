...
import { 
  Settings, X, RefreshCw, Moon, Sun, Download, Copy, Check, 
  Search, Mail, Plus, Settings2,
  ChevronRight, MoreHorizontal, DownloadCloud,
...
import { Breadcrumb, BreadcrumbList, BreadcrumbItem, BreadcrumbLink, BreadcrumbSeparator } from '../../../lib/ui/Breadcrumb';
    ...
    <div class="flex h-[calc(100vh-4rem)] overflow-hidden bg-bg animate-in fade-in duration-500">
      ...
      <aside class="w-80 border-r border-stroke bg-panel flex flex-col shrink-0 overflow-y-auto custom-scrollbar shadow-xl z-10">
        ...
        <div class="p-6 space-y-10 flex-1">
          {/* Base Palette */}
          <div class="space-y-4">
            <Label class="text-[10px] font-bold uppercase tracking-widest text-muted">Base Palette</Label>
            <div class="grid grid-cols-2 gap-2">
              <For each={BASE_COLORS}>
                {(color) => (
                  ...
                  >
                    <color.icon size={12} class={theme.baseColor() === color.value ? "text-primary" : ""} />
                    {color.name}
                  </button>
                )}
              </For>
          ...
          {/* Accent Color */}
          <div class="space-y-4">
             <Label class="text-[10px] font-bold uppercase tracking-widest text-muted">Primary Accent</Label>
        ...
        <div class="p-6 border-t border-stroke bg-surface/30 space-y-3">
          <Button class="w-full gap-2 text-xs font-black uppercase tracking-tighter" size="sm" onClick={() => setShowExport(true)}>
          ...
          <div class="space-y-8">
            ...
            <div class="flex items-end justify-between border-b-4 border-fg pb-8">
              <div class="space-y-4">
                <div class="flex items-center gap-4">
                  ...
                  <div class="flex -space-x-3">
                    ...
                    <div class="h-8 w-8 rounded-full bg-surface border-2 border-bg flex items-center justify-center text-[10px] font-bold">+5</div>
                  </div>
                </div>
                <div class="space-y-2">
              ...
              <div class="flex gap-3">
                 <Button variant="outline" size="sm" class="h-10 px-6 font-bold" style={{ "box-shadow": "var(--shadow-btn)" }}>Documentation</Button>
                 <Button size="sm" class="h-10 px-6 font-bold" style={{ "box-shadow": "var(--shadow-btn)" }}>Live API</Button>
              </div>
            </div>
          </div>
            ...
            <div class="lg:col-span-8 space-y-10">
              <div class="grid grid-cols-3 gap-6">
                <Card class="border-2 border-fg" style={{ "box-shadow": "var(--shadow-main)" }}>
                   <CardHeader class="pb-2"><CardTitle class="text-[10px] font-black uppercase text-muted">Traffic</CardTitle></CardHeader>
                   <CardContent>
                ...
                <Card class="border-2 border-fg" style={{ "box-shadow": "var(--shadow-main)" }}>
                   <CardHeader class="pb-2"><CardTitle class="text-[10px] font-black uppercase text-muted">Uptime</CardTitle></CardHeader>
                   <CardContent>
                ...
                <Card class="border-2 border-fg" style={{ "box-shadow": "var(--shadow-main)" }}>
                   <CardHeader class="pb-2"><CardTitle class="text-[10px] font-black uppercase text-muted">Servers</CardTitle></CardHeader>
                   <CardContent>
              ...
              <div class="grid grid-cols-2 gap-6">
                ...
                <Card class="border-2 border-fg" style={{ "box-shadow": "var(--shadow-main)" }}>
                   <CardHeader class="pb-4">
                     <div class="flex items-center justify-between">
                       <CardTitle class="text-[10px] font-black uppercase text-muted">Core 02</CardTitle>
                       <Badge variant="outline">Backup</Badge>
                     </div>
                   </CardHeader>
                   <CardContent class="space-y-4">
                     <div class="space-y-2">
                       <Skeleton class="h-4 w-full border border-stroke opacity-50" />
                       <Skeleton class="h-4 w-[60%] border border-stroke opacity-50" />
                     </div>
                     <div class="flex items-center justify-between pt-2">
                  ...
                  <Table>
                    <TableHeader class="bg-surface">
                      <TableRow class="border-b-2 border-stroke">
                        <TableHead class="pl-6 font-black uppercase text-[10px]">Event Source</TableHead>
                        <TableHead class="font-black uppercase text-[10px]">Severity</TableHead>
                        <TableHead class="pr-6 text-right font-black uppercase text-[10px]">Load</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      <TableRow class="border-b border-stroke/50 font-bold">
                        ...
                        <TableCell class="pr-6 text-right font-mono text-xs font-bold text-primary">0.12ms</TableCell>
                      </TableRow>
                      <TableRow class="border-b border-stroke/50 font-bold">
                        <TableCell class="pl-6 py-4">
                          <div class="flex items-center gap-2">
                        ...
                        <TableCell><Badge variant="primary">Syncing</Badge></TableCell>
                        <TableCell class="pr-6 text-right font-mono text-xs font-bold text-primary">42.8ms</TableCell>
                      </TableRow>
                    </TableBody>
                  </Table>
                </CardContent>
              ...
              <div class="flex flex-col gap-6 items-center">
                <div class="flex gap-4 items-center">
                   ...
                   <SegmentedControl 
                     options={[{ label: 'Real-time', value: 'rt' }, { label: 'Historical', value: 'hist' }, { label: 'Predicted', value: 'pred' }]} 
                     value="rt"
                     onChange={() => {}}
                ...
                <Tabs defaultValue="overview" class="w-full max-w-md">
                   <TabsList class="grid grid-cols-3 border-2 border-fg p-1 bg-surface rounded-none">
                      <TabsTrigger value="overview" class="text-[10px] font-black uppercase">Overview</TabsTrigger>
                      <TabsTrigger value="security" class="text-[10px] font-black uppercase">Security</TabsTrigger>
                      <TabsTrigger value="nodes" class="text-[10px] font-black uppercase">Nodes</TabsTrigger>
                   </TabsList>
                </Tabs>
              </div>
              ...
              <div class="space-y-6 pt-10 border-t-4 border-fg">
                 ...
                 <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <Alert variant="success" class="border-2 border-fg rounded-none">
                       ...
                       <AlertDescription class="text-[10px] font-bold">All encrypted volumes synced to secondary regions.</AlertDescription>
                    </Alert>
                    <Alert variant="warning" class="border-2 border-fg rounded-none">
                       <AlertTitle class="font-black uppercase text-[10px]">API Latency Spike</AlertTitle>
                       <AlertDescription class="text-[10px] font-bold">Inbound request threshold exceeded in EU-WEST-1.</AlertDescription>
                    </Alert>
                    <Alert variant="error" class="border-2 border-fg rounded-none">
                       <AlertTitle class="font-black uppercase text-[10px]">Core Protocol Failure</AlertTitle>
                       <AlertDescription class="text-[10px] font-bold">Critical error detected in synchronization logic.</AlertDescription>
                    </Alert>
                    <Alert variant="default" class="border-2 border-fg rounded-none">
                       <AlertTitle class="font-black uppercase text-[10px]">Maintenance Window</AlertTitle>
                       <AlertDescription class="text-[10px] font-bold">Scheduled updates starting at 02:00 UTC.</AlertDescription>
                    </Alert>
                 </div>
              </div>
            </div>
            ...
            <div class="lg:col-span-4 space-y-10">
               <Card class="border-2 border-fg bg-panel" style={{ "box-shadow": "var(--shadow-main)" }}>
                 ...
                 <CardContent class="space-y-6 pt-6">
                   <div class="space-y-2">
                     <Label class="text-[10px] font-black uppercase">Cluster Identifier</Label>
                     <Input value="starling-production-01" class="border-2 border-fg rounded-none font-mono" />
                   </div>
                   ...
                   <div class="space-y-3">
                     <div class="flex items-center justify-between">
                       ...
                       <span class="text-[10px] font-mono font-bold">65%</span>
                     </div>
                     <Progress value={65} class="h-2 border border-fg rounded-none overflow-hidden" />
                   </div>

                   <div class="space-y-3">
                      <Label class="text-[10px] font-black uppercase">Scaling Threshold</Label>
                   ...
                   <div class="space-y-4 pt-2">
                      <div class="flex items-center space-x-2">
                        ...
                        <Label for="stealth" class="text-[10px] font-black uppercase leading-none">Stealth Protocol</Label>
                      </div>
                      <div class="flex items-center space-x-2">
                        <Checkbox id="backup" />
                        <Label for="backup" class="text-[10px] font-black uppercase leading-none">Auto-Backup</Label>
                      </div>
                   </div>
                   ...
                   <div class="space-y-3">
                     ...
                     <RadioGroup defaultValue="us-east" name="region" onValueChange={() => {}}>
                        <div class="flex items-center gap-2">
                           ...
                           <Label for="r1" class="text-[10px] font-bold">US-EAST</Label>
                        </div>
                        <div class="flex items-center gap-2">
                           <RadioGroupItem value="eu-west" id="r2" />
                           <Label for="r2" class="text-[10px] font-bold">EU-WEST</Label>
                        </div>
                     </RadioGroup>
                   </div>
                   ...
                   <Accordion multiple class="border-t border-stroke">
                     <AccordionItem value="details" class="border-b border-stroke">
                       <AccordionTrigger class="text-[10px] font-black uppercase py-3">Advanced Parameters</AccordionTrigger>
                       <AccordionContent class="pb-4 text-[10px] font-bold text-muted">
                         Encryption levels and proxy rotation settings are managed automatically by the core cluster.
                       </AccordionContent>
                     </AccordionItem>
                   </Accordion>
                 </CardContent>
                 <CardFooter>
                    <Button class="w-full h-12 font-black uppercase tracking-widest border-2 border-fg" style={{ "box-shadow": "var(--shadow-btn)" }}>Commit Changes</Button>
                 </CardFooter>
               </Card>
      ...
      <Modal isOpen={showExport()} onClose={() => setShowExport(false)}>
        ...
        <ModalFooter>
          <Button variant="outline" class="font-bold" onClick={() => setShowExport(false)}>Close</Button>
          <Button onClick={copyToClipboard} class="gap-2 min-w-[140px] font-black uppercase">
            <Show when={hasCopied()} fallback={<><Copy size={16} /> Copy CSS</>}>
